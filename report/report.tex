...
\begin{abstract}
Diabetes mellitus is a leading cause of preventable long-term complications including
cardiovascular disease, chronic kidney disease, and peripheral neuropathy.
Traditional clinical screening relies on single-visit measurements and fixed thresholds,
which may fail to capture the progressive nature of diabetic complications.
This project develops a multi-label machine learning system that simultaneously
predicts the risk of three complication types from longitudinal clinical records
derived from the UCI Diabetes 130-US Hospitals dataset (101,766 encounters,
71,518 unique patients).
Three classifiers were trained and compared: ClassifierChain (Random Forest),
MultiOutputClassifier (Gradient Boosting), and a PyTorch Neural Network with
Focal BCE loss for imbalanced learning.
After threshold optimisation, the ClassifierChain (Random Forest) achieved
\textbf{Recall-Macro = 0.878} across all three labels simultaneously---the primary
clinical objective of minimising missed complication cases.
\end{abstract}

\vspace{0.5em}
\begin{center}
\small\textbf{Deployment URL:}~\url{https://huggingface.co/spaces/samishah2004/dcrp}
\end{center}
...
Hyperparameters were tuned manually by holding out a fixed 20\% test set and optimising
for Recall-Macro~$\geq 0.80$ while maintaining reasonable F1-Macro. Tables~\ref{tab:hp_rf},
\ref{tab:hp_gbm}, and~\ref{tab:hp_nn} document the configurations explored for each model
and justify the final selections.

\begin{table}[H]
  \centering
  \caption{Hyperparameter Search --- ClassifierChain (Random Forest)}
  \label{tab:hp_rf}
  \begin{tabular}{cccccl}
    \toprule
    n\_estimators & max\_depth & min\_samples\_leaf & Recall-Macro & F1-Macro & Selected \\
    \midrule
    100 & 10 & 5 & 0.821 & 0.338 & \\
    150 & 12 & 4 & 0.849 & 0.347 & \\
    \textbf{200} & \textbf{14} & \textbf{4} & \textbf{0.878} & \textbf{0.362} & \checkmark \\
    200 & None & 2 & 0.871 & 0.341 & \\
    \bottomrule
  \end{tabular}
  \begin{flushleft}
  \small \textit{Justification:} \texttt{n\_estimators=200} provides strong ensemble stability.
  \texttt{max\_depth=14} limits overfitting while capturing complex longitudinal feature
  interactions. \texttt{class\_weight="balanced"} is essential given neuropathy's 0.8\%
  prevalence and was kept fixed across all configurations.
  \end{flushleft}
\end{table}

\begin{table}[H]
  \centering
  \caption{Hyperparameter Search --- MultiOutputClassifier (Gradient Boosting)}
  \label{tab:hp_gbm}
  \begin{tabular}{cccccl}
    \toprule
    n\_estimators & learning\_rate & max\_depth & subsample & Recall-Macro & Selected \\
    \midrule
    100 & 0.10 & 3 & 1.00 & 0.298 & \\
    100 & 0.05 & 4 & 0.85 & 0.311 & \\
    \textbf{120} & \textbf{0.10} & \textbf{4} & \textbf{0.85} & \textbf{0.313} & \checkmark \\
    150 & 0.10 & 5 & 0.85 & 0.309 & \\
    \bottomrule
  \end{tabular}
  \begin{flushleft}
  \small \textit{Justification:} GBM cannot recover neuropathy recall even after threshold
  tuning due to the absence of focal loss weighting. Retained as a comparison baseline.
  \texttt{subsample=0.85} reduces variance without sacrificing predictive performance.
  A higher \texttt{learning\_rate} combined with more trees (150) led to overfitting on
  the minority label.
  \end{flushleft}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{Hyperparameter Search --- PyTorch Neural Network (MLP)}
  \label{tab:hp_nn}
  \begin{tabular}{cccccccl}
    \toprule
    LR & Batch & Dropout & Epochs & $\gamma$ & pw\_scale & Recall-Macro & Selected \\
    \midrule
    1e-3 & 128 & 0.20 & 50  & 2.0 & 2.0 & 0.831 & \\
    3e-4 & 64  & 0.30 & 70  & 2.0 & 2.5 & 0.852 & \\
    \textbf{3e-4} & \textbf{128} & \textbf{0.30} & \textbf{70} & \textbf{2.0} & \textbf{2.5} & \textbf{0.853} & \checkmark \\
    1e-4 & 128 & 0.30 & 100 & 3.0 & 3.0 & 0.848 & \\
    \bottomrule
  \end{tabular}
  \begin{flushleft}
  \small \textit{Justification:} \texttt{lr=3e-4} with AdamW and cosine annealing provides
  stable convergence without oscillation. \texttt{dropout=0.30} combined with BatchNorm
  prevents the overfitting observed at lower regularisation. \texttt{gamma=2.0} in Focal
  loss gives the best recall/precision balance --- higher $\gamma$ over-suppresses easy
  negatives and degrades kidney recall. \texttt{pos\_weight\_scale=2.5} amplifies the
  positive-class signal across all three labels; scaling above 3.0 caused instability
  in the cardiovascular label. pw\_scale abbreviates \texttt{pos\_weight\_scale}.
  \end{flushleft}
\end{table}
...
providing accountability for all deliverables (Phases 1--5).
...
\midrule
\multirow{3}{*}{Deployment} & Streamlit application (app.py) & Maheer Khurram \\
...
\begin{itemize}
    \item \textbf{Abdul Wahab}: Data acquisition, data cleaning \& preprocessing,
          threshold tuning framework, confusion matrix analysis, inference pipeline
          \& feature reconstruction, LaTeX report compilation.
    ...
    \item \textbf{Maheer Khurram}: ClassifierChain (Random Forest) implementation,
          hyperparameter optimisation, ROC curves \& visualisation, feature importance
          analysis, Streamlit application development,
...
\section*{Deliverable 3: Deployment and Final Technical Report}
\addcontentsline{toc}{section}{Deliverable 3: Deployment and Final Technical Report}
...
The trained ClassifierChain (Random Forest) model was packaged into a Streamlit web
application and deployed on \textbf{Hugging Face Spaces} for public access.
The application provides a real-time clinical decision-support interface allowing
users to enter patient data and receive simultaneous risk scores for all three
diabetes complications.

\begin{center}
\fbox{\parbox{0.85\textwidth}{%
    \centering
    \textbf{Live Application URL} \\[0.3em]
    \url{https://huggingface.co/spaces/samishah2004/dcrp}
}}
\end{center}
...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, rounded corners=4pt, draw=#1, fill=#1!10,
                text width=3.0cm, align=center, font=\small, minimum height=1cm},
    arr/.style={->, thick, #1},
]
    \node[box=blue!60]  (ui)     {Streamlit UI\\(app.py)};
    \node[box=teal!60,  right=of ui]  (feat)   {Feature\\Engineering};
    \node[box=orange!60,right=of feat] (model)  {ClassifierChain\\Random Forest};
    \node[box=green!60, right=of model] (out)   {Risk Scores\\(3 labels)};

    \draw[arr=blue!60]   (ui)    -- node[above,font=\scriptsize]{Clinical inputs} (feat);
    \draw[arr=teal!60]   (feat)  -- node[above,font=\scriptsize]{Feature vector}  (model);
    \draw[arr=orange!60] (model) -- node[above,font=\scriptsize]{Probabilities}   (out);
\end{tikzpicture}
\caption{Real-time inference pipeline in the deployed application.}
...
\begin{itemize}
    \item \textbf{Input Panel (left)}: Collects 18 clinical parameters grouped into
          four sections --- Demographics, Laboratory Results, Hospital Utilisation, and
          Medications. All inputs use validated widgets (sliders, dropdowns, number
          fields) to prevent erroneous entries.

    \item \textbf{Results Panel (right)}: Displays colour-coded risk cards for each
          complication with a probability percentage, a visual progress bar, and a
          risk badge (\textit{Low / Moderate / High}) relative to the tuned threshold.
          A summary card identifies all flagged complications.
\end{itemize}

\subsection{Feature Reconstruction at Inference}

During deployment, the model expects the same longitudinal feature vector produced
by \texttt{feature\_engineering.py}. For single-visit inputs, the app constructs
the full vector by setting \texttt{\_std = \_delta = \_slope = 0} and assigning
the entered value to \texttt{\_mean = \_min = \_max = \_last}. The same
\texttt{StandardScaler} fitted during training (\texttt{scaler\_final.pkl}) is
then applied only to the 76 numeric columns before inference, leaving the
one-hot encoded categorical columns unscaled.

\subsection{Deployment Configuration}

The Space runs on the \textbf{Streamlit SDK} provided by Hugging Face.
...
Model files exceed GitHub's standard 50\,MB limit and are stored using
\textbf{Git Large File Storage (LFS)} in both the GitHub repository and
...
Table~\ref{tab:final_compare} summarises model performance before and after
threshold tuning on the held-out test set.

\begin{table}[H]
\centering
\caption{Final model comparison (test set, 20\% hold-out)}
\label{tab:final_compare}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{F1-Micro} & \textbf{F1-Macro} & \textbf{Recall-Macro} & \textbf{AUC-Macro} \\
\midrule
Chain RF (base)     & 0.606 & 0.343 & 0.394 & 0.708 \\
MO GBM (base)       & 0.726 & 0.297 & 0.313 & 0.724 \\
Neural Net (base)   & 0.464 & 0.336 & 0.916 & 0.707 \\
\midrule
Chain RF (tuned)    & 0.495 & 0.362 & \textbf{0.878} & 0.708 \\
...
The \textbf{ClassifierChain RF with tuned thresholds} was selected as the final
deployed model. It is the only configuration that achieves Recall $\geq 0.80$
\emph{simultaneously} for all three labels while maintaining competitive
F1-Macro performance.

\subsection{Ablation: Impact of Longitudinal Features}

To quantify the contribution of longitudinal feature aggregation, a baseline
ClassifierChain was trained using only the static (last-visit) values without
\texttt{\_slope}, \texttt{\_delta}, or \texttt{\_std} features.

\begin{table}[H]
\centering
\caption{Ablation study --- longitudinal features vs.\ static-only}
\label{tab:ablation}
\begin{tabular}{lccc}
\toprule
\textbf{Feature Set} & \textbf{F1-Macro} & \textbf{Recall-Macro} & \textbf{AUC-Macro} \\
\midrule
Static only (last visit)          & 0.328 & 0.831 & 0.681 \\
\textbf{Full longitudinal (ours)} & \textbf{0.362} & \textbf{0.878} & \textbf{0.708} \\
\midrule
$\Delta$                          & +0.034 & +0.047 & +0.027 \\
\bottomrule
\end{tabular}
\end{table}

Longitudinal features (\texttt{\_slope}, \texttt{\_delta}, \texttt{\_std}) provide
a consistent improvement across all metrics, confirming that disease trajectory
information contributes meaningfully beyond single-visit snapshots.

\subsection{Ablation: Impact of Threshold Optimisation}

Using a fixed threshold of 0.50 across all labels produced near-zero recall for
neuropathy (prevalence = 0.8\%). Per-label threshold tuning to maximise
Recall $\geq 0.80$ recovered neuropathy recall from 0.03 to 0.87 at the cost
of a moderate reduction in precision. This precision--recall tradeoff is
explicitly acceptable in a clinical screening context where false negatives
(missed complications) carry greater consequence than false positives.
...
\begin{enumerate}
    \item \textbf{Synthetic longitudinal ordering.} Encounters are sorted by
          encounter ID as a proxy for visit order; true date-stamped data would
          produce more reliable slope and delta features.

    \item \textbf{Severe neuropathy imbalance.} At 0.8\% prevalence,
          neuropathy recall is sensitive to threshold choice and cannot be
          substantially improved without oversampling (e.g., SMOTE) or
          additional training data.

    \item \textbf{Inference on single-visit patients.} The app reconstructs
          longitudinal features assuming one visit, zeroing out all trend
          statistics. Multi-visit patients would benefit from richer features
          but require multiple inputs.

    \item \textbf{No prospective validation.} All evaluation is performed on
          a held-out split of the same dataset. External validation on an
          independent clinical cohort is required before any real-world use.
\end{enumerate}

\subsection{Future Directions}

\begin{itemize}
    \item Apply SMOTE or class-conditional oversampling to improve neuropathy recall.
    \item Incorporate actual visit timestamps to improve longitudinal feature quality.
    \item Explore transformer-based sequential models (e.g., Time-Series Transformer)
          over the visit sequence rather than manual aggregation.
    \item Extend to additional complication types (retinopathy, foot ulcers).
    \item Conduct external validation on an independent clinical dataset.
\end{itemize}
